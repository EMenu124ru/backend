...
from apps.orders.factories import DishFactory, DishImageFactory
...
def test_create_dish_images_by_manager(
    ...
    dish_images = DishImageFactory.build_batch(size=COUNT_IMAGES)
    data = {
    ...
    assert DishImage.objects.filter(dish__id=dish.id).exists()
    assert DishImage.objects.filter(dish__id=dish.id).count() == COUNT_IMAGES
...
def test_delete_dish_images_by_manager(
    ...
    dish = DishFactory.create()
    dish_images = DishImageFactory.create_batch(size=COUNT_IMAGES, dish=dish)
    api_client.force_authenticate(user=manager.user)
    ...
    assert not DishImage.objects.filter(
...
def test_delete_dish_images_by_waiter(
    ...
    assert DishImage.objects.filter(
        id__in=[image.id for image in dish_images]
    ).exists()
