...
tests:  ##@Testing Test application with pytest
	make docker-django-run "pytest --verbosity=2 --showlocals --log-level=DEBUG --full-trace $(args)"

tests-cov:  ##@Testing Test application with pytest and create coverage report
	make docker-django-run "coverage run -m pytest --cov-config=setup.cfg" && make docker-django-run "coverage html"
